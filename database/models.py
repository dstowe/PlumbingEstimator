...
# WBS Category Functions
def create_default_wbs_categories(project_id):
    """Create default WBS categories for a new project with Base Bid parent"""
    db = get_db()
    cursor = db.cursor()
    ...
    cursor.execute(
        ...
        (project_id, None, 'Base Bid', 0)
    )
    base_bid_id = cursor.lastrowid
    
    # Create child categories under Base Bid
    child_categories = [
        'UG Water',
        'UG Sanitary', 
        'UG Storm',
    ...
    for i, name in enumerate(child_categories):
        cursor.execute(
            'INSERT INTO wbs_categories (project_id, parent_id, name, sort_order) VALUES (?, ?, ?, ?)',
            (project_id, base_bid_id, name, i)
        )
...
def delete_wbs_category(category_id):
    """Delete a WBS category (only if no items assigned and no children)"""
    db = get_db()
...
def check_wbs_category_has_items(category_id):
    """Check if a WBS category has any items assigned to it"""
    db = get_db()
    
    count = db.execute(
        'SELECT COUNT(*) as count FROM detected_items WHERE wbs_category_id = ?',
        (category_id,)
    ).fetchone()['count']
    
    return count > 0

def check_wbs_category_has_children(category_id):
    """Check if a WBS category has any child categories"""
    db = get_db()
    
    count = db.execute(
        'SELECT COUNT(*) as count FROM wbs_categories WHERE parent_id = ?',
        (category_id,)
    ).fetchone()['count']
...
def get_wbs_path(category_id):
    """Get the full path of a category (e.g., 'Base Bid > UG Water')"""
    db = get_db()
